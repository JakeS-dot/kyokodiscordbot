...
# TOKEN FOR KYOKO (TEST) IS ODQ5NjEzOTEyMDI1OTg5MTUw.YLdunQ.td3lhcTYICMpT3VhTVOaY6aBQAo


TOKEN = 'ODQ5NjEzOTEyMDI1OTg5MTUw.YLdunQ.td3lhcTYICMpT3VhTVOaY6aBQAo'
...
async def changelog(ctx):
    ...
    changelog_embed.add_field(name="We just did a lot fo stuff involving vc's",
...
async def wallet(ctx):
    ...
    embed = discord.Embed(title=str(ctx.author.nickname) + "'s Wallet!!", color=0xa00000)
...
async def play(ctx, *, query: t.Optional[str]):
    ...
    try:
        ...
        print("But song is already playing")
        return
...
mango_url = "mongodb+srv://s9158257:s9158257@animecards.687qc.mongodb.net/test"
cluster = MongoClient(mango_url)
db = cluster["AnimeTest"]
...
@client.command()
async def draw(ctx):
    post = {'_id': ctx.author.id}, {'$inc': {'score': 1}}
